...
Unity 的优势是完整、成熟、适合做真正的 VR 应用；Web 的优势是轻、快、好传播。我们想逃离 Unity，不是因为 Unity 不行，而是因为这个项目如果只是“看建筑、逛展馆、做轻交互”，每次都打 APK 真的太折磨。

现在 Web 里放真实建筑，主要有三条路：

| 路线 | 典型格式 | 优点 | 缺点 | 适合场景 |
|---|---|---|---|---|
| 网格模型 | `.glb` / `.gltf` | Web 支持最好，Three.js 生态成熟，容易进 WebXR | 大场景高精度时面数和贴图会爆 | 展馆、建筑外立面、教学展示、VR 漫游 |
| 点云 | `.las` / `.laz` / COPC / EPT / Potree | 适合测绘和大尺度空间，保留真实采样点 | 表面不是连续网格，视觉上有颗粒感 | GIS、测量、BIM 对照、海量扫描数据浏览 |
| 3D 高斯泼溅 | `.ply` / `.splat` / `.spz` / `.ksplat` / `.sog` | 照片级观感强，植物、细碎结构、复杂材质很舒服 | 格式仍在快速变化，编辑、碰撞、测量不如网格成熟 | 写实展示、数字孪生预览、文化遗产/建筑外观复现 |

如果是给普通网页或 WebXR 展馆（尤其适用于我们接下来开发的项目）用，我们现在更推荐这样搞：

**主路线用 GLB 网格模型**：RealityCapture/RealityScan 出高模，简化、重投影贴图，再转 GLB。

不要一开始就纠结“哪个技术最先进”。先问几个更朴素的问题：
...
很多项目最后不是死在算法上，而是死在一个 800MB 的模型、一个加载半分钟的网页，以及一句“我这边打不开”。
...
1. PC Chrome 能打开，并且测试一轮chrome浏览器自带的lightroom测试分数达到60分以上。
...
如果现在让我做一个“校园建筑 WebXR 还原”项目，我会这样走。注意，这不是“唯一正确路线”，而是比较适合交接、展示和后续维护的懒人路线：

1. 无人机拍照：正射 + 斜拍 + 立面环绕。
2. 视频抽帧只作为补充，优先用清晰照片。
3. RealityCapture/RealityScan 对齐照片。
4. 框选重建区域。
5. 生成高模。
6. 清理飞面和无关区域。
7. 简化到 100k 到 500k 三角面。
8. 重新 unwrap 和贴图。
9. 导出 OBJ/FBX/GLB。
10. Blender 检查比例、原点、朝向、材质。
11. glTF-Transform 压缩 GLB。
12. 放进 Three.js/WebXR 模板。
13. PC、手机、Quest 3 全部测试。
14. 如果追求照片级外观，再额外做 3DGS 展示版本。
15. 如果追求测绘档案，再额外保留 LAZ/COPC/EPT 点云版本。

最后记住一句话：**Web 端不是不能放大场景，而是要按 Web 的方式组织资产。**

网格要简化，贴图要压缩，点云要分层，高斯泼溅要选对格式和 viewer。把这些想清楚，现实建筑进网页这件事就没有那么玄学了。

Unity 那套工程当然还能继续维护，但如果只是想做一个能传播、能打开、能让人快速看到效果的建筑展馆，Web + 3DGS/GLB 这条路确实值得试。极客湾已经把办公室做成赛博展馆给我们看过一次了，我们没必要闭门造车。先摸着他们过河，跑通，再慢慢加自己的东西。
